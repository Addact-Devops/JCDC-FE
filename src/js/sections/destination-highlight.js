...
window.DestinationHighlight = (function () {
    ...
    function animateCount(el, target) {
        ...
        function step(timestamp) {
            ...
            el.textContent = el.dataset.countPrefix + current.toLocaleString("en-US") + el.dataset.countSuffix;
            if (progress < 1) {
    ...
    function initReveal(root) {
        ...
        const stats = [...container.querySelectorAll(".destination-highlight__stat")];
    ...
    function initBodyReveal(root) {
        ...
        // rootMargin shrinks the viewport's bottom edge by 45%, so the body
        // only counts as "in view" once it has been scrolled up past
        // roughly the middle of the screen — same as .jcdc / .our-story.
        const observer = new IntersectionObserver(
            (entries) => {
                entries.forEach((entry) => {
                    if (!entry.isIntersecting) return;
